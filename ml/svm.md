...
![](images/2021-07-29-19-36-10.png)
...
若满足$\alpha_i > 0$，进一步地，
- 若$0<\alpha_i<C$, 则$\beta_i=0$，即刚好$y_i(W^Tx_i+b)=1$，样本恰好在最大间隔边界上
- 若$\alpha_i=C$，有$\beta_i>0$，此时若$\beta_i<1$则该样本落在最大间隔内部
- 若$\beta_i>1$则该样本落在最大间隔内部（不属于自己的另一部分）即被错误分类
如下图所示：
![](images/2021-07-29-19-44-21.png)
####  为什么SVM对缺失数据敏感？
